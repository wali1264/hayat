...
const Dashboard: React.FC<DashboardProps> = ({ drugs, orders, customers, onNavigate }) => {
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-xl shadow-lg">
                    <h3 className="text-lg font-bold text-gray-800 mb-4">هشدار کمبود موجودی</h3>
                    {lowStockItems.length > 0 ? (
                        <ul className="divide-y divide-gray-200">
                            {lowStockItems.map(drug => (
                                <li key={drug.id} className="py-3 flex justify-between items-center">
                                    <span className="text-gray-700 font-medium">{drug.name}</span>
                                    <span className="text-red-600 font-bold bg-red-100 px-3 py-1 rounded-full text-sm">{drug.quantity} عدد</span>
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-center text-gray-500 py-8">هیچ دارویی در حال اتمام نیست.</p>
                    )}
                     <button onClick={() => onNavigate('inventory')} className="mt-4 w-full py-2 text-center text-teal-600 font-semibold bg-teal-50 rounded-lg hover:bg-teal-100 transition-colors">
                        مشاهده کامل انبار
                    </button>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-lg">
                    <h3 className="text-lg font-bold text-gray-800 mb-4">آخرین سفارشات در انتظار آماده‌سازی</h3>
                    {orders.filter(o => o.status === 'در حال پردازش').slice(0, 5).length > 0 ? (
                        <ul className="divide-y divide-gray-200">
                            {orders.filter(o => o.status === 'در حال پردازش').slice(0, 5).map(order => (
                                <li key={order.id} className="py-3 flex justify-between items-center">
                                    ...
                                    <span className="text-gray-800 font-semibold">{order.totalAmount.toLocaleString()} <span className="text-sm font-normal">افغانی</span></span>
                                </li>
                            ))}
                        </ul>
                    ) : (
                         <p className="text-center text-gray-500 py-8">هیچ سفارش در حال پردازشی وجود ندارد.</p>
                    )}
                    <button onClick={() => onNavigate('fulfillment')} className="mt-4 w-full py-2 text-center text-teal-600 font-semibold bg-teal-50 rounded-lg hover:bg-teal-100 transition-colors">
                        مشاهده همه سفارشات
                    </button>
                </div>
             </div>
